...
| `expo-modules-core+57.0.17.patch` | `expo-modules-core@57.0.17` |
...
基类 `Exception` 已是 `@unchecked Sendable`，Swift 6.2 要求子类再写一遍。`EventEmitter` 把 weak emitter 送进 `@JavaScriptActor` 时，6.2 的 sending 检查会报错。`57.0.17` 仍是同一套源码，官方同样要求升 Xcode 26.4+（[expo/expo#47539](https://github.com/expo/expo/issues/47539)）。
...
1. 删对应的 `.patch`（`expo-modules-jsi+57.1.0.patch` 和/或 `expo-modules-core+57.0.17.patch`）
