...
OUTPUT_NAME=${OUTPUT_NAME:-$(printf '%s-device-%s-%s' "$DEFAULT_OUTPUT_BASENAME" "$(printf '%s' "$CONFIGURATION" | tr '[:upper:]' '[:lower:]')" "$(date +%Y%m%d%H%M%S)" | tr ' ' '-')}
...
log "Building unsigned device IPA for $SCHEME_NAME ($CONFIGURATION) on $DESTINATION..."
...
log "Unsigned device IPA created: $IPA_PATH"
